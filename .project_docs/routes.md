...
- Nota: `/sin-acceso`/`/not-access` **no tiene página propia todavía** (`VUE_ROUTER_R0004` en el log al redirigir ahí) — gap preexistente heredado de `users`/`dashboard.ts` (la página nunca se portó en ninguna fase), no algo introducido por `news`.
...
- `/panel/preguntas-frecuentes` (dashboard) sigue devolviendo un redirect de guard sin sesión (degrada a `404` porque `/sin-acceso`/`/not-access` no tiene página propia — gap preexistente heredado, no introducido aquí, ver sección de `news` dashboard arriba) — confirma que el rename `pages.faqs`→`pages.dashboard_faqs` no rompió el guard ni el dashboard.
