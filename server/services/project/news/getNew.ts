...
// Port de src/services/project/news/getNewById.ts (Next) — la variante
// dashboard (por id numérico, con token), no la pública por slug. Se nombra
// `getNew` (no `getNewById`) siguiendo el mismo criterio que `getUser.ts`
// (singular del dominio, sin sufijo "ById").
